...
LOADER_DICT = {"UnstructuredHTMLLoader": ['.html'],
               ...
               "CustomExcelLoader": [".xlsx",".xls"],
               #"UnstructuredExcelLoader": ['.xlsx', '.xls', '.xlsd'],
               "CSVLoader": [".csv"],
               # "FilteredCSVLoader": [".csv"], # 需要自己指定，目前还没有支持
               "RapidOCRPDFLoader": [".pdf"],       #"PyPDFLoader": [".pdf"],
               "TextLoader": [".wiki"],
               "MediaLoader": [".mp4"],
...
LOADER_DICT2 = {"UnstructuredFileLoader": ['.eml', '.html', '.json', '.md', '.msg', '.rst',
                                          '.rtf', '.txt', '.xml',
                                          '.doc', '.docx', '.epub', '.odt',
...
class CustomExcelLoader(langchain.document_loaders.csv_loader.CSVLoader):
    def __init__(self,file_path: str):
        try:
            df = pd.read_excel(file_path,header=None)
            header_row = df.iloc[:5].apply(lambda row: row.notnull().sum(), axis=1).idxmax()
        ...
        df.to_csv(f.name)
        langchain.document_loaders.csv_loader.CSVLoader.__init__(self,f.name)
...
def get_loader(loader_name: str, file_path_or_content: Union[str, bytes, io.StringIO, io.BytesIO]):
    ...
    '''
    try:
        if loader_name in ["RapidOCRPDFLoader", "RapidOCRLoader", "MediaLoader","FilteredCSVLoader"]:
    ...
    elif loader_name == "CustomExcelLoader":
...
class KnowledgeFile:
    ...
    def docs2texts(
        ...
        if not docs:
            return []
        if self.ext in [".csv",'.wiki','.xlsx','.xls']:        #.html
            pass
        elif self.ext == '.md':     #注意，这个跟下面的MarkdownHeaderTextSplitter没关系，MarkdownHeaderTextSplitter是个全局设置，不能根据扩展名修改
            text_splitter = langchain.text_splitter.CharacterTextSplitter(separator = "\n\n",chunk_size = chunk_size,chunk_overlap  = chunk_overlap)
